# ego Graphica - 完全設計ドキュメント

> 各ドキュメントの乖離を解消し、統合した最終仕様
...
| ego Graphicaエージェント | アーティストの代わりに営業・接客を行うAIエージェント |
...
## 9.2 CAG（Context Augmented Generation）
...
エージェントが会話中に実行できるツール。Vercel AI SDKのtool関数とZodを使用。
...
**戻り値**: success, works, count, displayComponent="PortfolioGrid"

### checkAvailability

アーティストのスケジュール空き状況を確認。

**パラメータ**:
- month: string（YYYY-MM形式）
- projectType?: "illustration" | "mural" | "collaboration" | "other"

**戻り値**: success, month, availability, message, leadTime, displayComponent="AvailabilityCalendar"
...
**戻り値**: success, quote, displayComponent="QuotePreview"
...
**パラメータ**:
- prefillData?: { projectType?, budget?, deadline?, description? }

**戻り値**: success, message, prefillData, displayComponent="ContactForm"

### searchWorks

特定のキーワードや要望に合った作品を検索。RAGベースのセマンティック検索。

**パラメータ**:
- query: string（自然言語の検索クエリ）
- types?: ("work" | "url" | "file")[]

**戻り値**: success, results, count, displayComponent="SearchResults"
...
| 項目 | 採用元 | 内容 |
|------|--------|------|
| エージェントの役割 | BLUEPRINT | 営業を行う自律型AIエージェント |
| データ構造 | user/REFACTOR | バケット名によるネスト構造 |
| 文字起こしモデル | user | `gpt-4o-transcribe` |
| Embedding次元数 | BLUEPRINT | 3072次元（text-embedding-3-large） |
| Pinecone Index名 | REFACTOR | `egographica` |
| Tool Calling | BLUEPRINT | showPortfolio, checkAvailability, generateQuote, showContactForm, searchWorks |
| API設計 | REFACTOR | `/auth/*`, `/persona`, `/chat`, `/hearing` 等 |
| 画面構成 | user | 6画面（登録、情報提供、URL提供、作品管理、エージェント会話、ヒアリング） |
| 拡張機能の範囲 | REFACTOR | ペルソナ設定、CAG、Tool Calling、マルチモーダルRAG |
| データモデル | user/REFACTOR | Profile, Persona, Work, FromUrl, Session, Hearing |
| UIライブラリ | user | shadcn-vue |
| 認証フロー | BLUEPRINT | firebase-admin経由、カスタムトークン方式 |
| Vector ID形式 | REFACTOR | `work_{workId}`, `url_{urlId}`, `file_{fileId}` |
| ディレクトリ構成 | REFACTOR | apps/web, apps/api, packages/shared |
| 環境変数 | REFACTOR | firebase-adminのみ（クライアントFirebase設定なし） |

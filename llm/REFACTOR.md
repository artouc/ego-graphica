...
## 4.1 Profile（アーティスト基本情報）

ファイル: packages/shared/src/types/profile.ts

**Profile型**:
- artist_name: string（アーティスト名）
- bucket_name: string（バケット名、一意識別子）
- created_at: Timestamp
- updated_at: Timestamp

## 4.2 Persona（ペルソナ設定）- 追加機能

ファイル: packages/shared/src/types/persona.ts

**Persona型**:
- character_name?: string（エージェント名）
- motif: string（モチーフ）
- tone: PersonaTone（話し方）
- personality: string[]（性格特性）
- artistic_philosophy: string（創作哲学）
- influences: string[]（影響を受けた作家・文化）
- keywords: string[]（よく使う言葉）
- greeting_style: string（挨拶の仕方）
- sample_responses: SampleResponse[]（理想的な応答例）
- avoid_topics: string[]（避けるべきトピック）
- backstory: string（バックストーリー）

**PersonaTone型**: "formal" | "friendly" | "artistic" | "professional" | "playful"

**SampleResponse型**:
- situation: string（状況説明）
- customer_message: string（顧客の発言例）
- ideal_response: string（理想的な応答）
...
- file_type: FileType（ファイル形式）
- title: string（タイトル）
- created_date: Timestamp（作成年月日）
- work_type: WorkType（クライアントワーク/自主制作）
- client_name?: string（クライアント名）
- sales_status: SalesStatus（販売状況）
- description?: string（説明）
- production_story?: string（制作ストーリー）
- image_analysis?: ImageAnalysis（Claude Vision解析結果）- 追加機能
- created_at: Timestamp
- updated_at: Timestamp

**FileType型**: "png" | "jpg" | "wav" | "mp4"

**WorkType型**: "client" | "self_initiated"

**SalesStatus型**: "sold" | "reserved" | "for_sale" | "not_for_sale"

**ImageAnalysis型**（マルチモーダルRAG用）:
...
- content: string
- tool_calls?: ToolCallResult[]（Tool Calling結果）
- created_at: Timestamp
